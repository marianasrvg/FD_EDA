...
void add_vect(Mesh m, float x, float y, float z){
    ...
    //aumentar el tamaño con realloc
    m->arrayCord = (COORD*)realloc(m->arrayCord, (m->nVert + 1)*sizeof(COORD));
...
void add_triangle(Mesh m, int point1, int point2, int point3){
    ...
    //Aumentar el tamaño del arreglo para el nuevo triangulo
    m->arrayTri = (Triangle*)realloc(m->arrayTri, (m->nFaces + 1)*sizeof(Triangle));
...
void tri_vecinos (Mesh m){
	...
	for (i=0; i<m->nFaces; i++){
		...
		for (x = 0; x<m->nFaces; x++){
			if (x != i){
			if ((m->arrayTri[i].p1 == m->arrayTri[x].p2)||(m->arrayTri[i].p1 == m->arrayTri[x].p1)
					|| (m->arrayTri[i].p1 == m->arrayTri[x].p3)){
				printf("%d\t",x);
			} else if ((m->arrayTri[i].p2 == m->arrayTri[x].p2)||(m->arrayTri[i].p2 == m->arrayTri[x].p1)
					|| (m->arrayTri[i].p2 == m->arrayTri[x].p3)){
				printf("%d\t",x);
			} else if ((m->arrayTri[i].p3 == m->arrayTri[x].p2)||(m->arrayTri[i].p3 == m->arrayTri[x].p1)
					|| (m->arrayTri[i].p3 == m->arrayTri[x].p3)){
				printf("%d\t",x);
			}
			}
		}
...
void find_tri (Mesh m, int point1, int point2, int point3){
	int i;
	for ( i = 0; i < m->nFaces; i++){
		if ((m->arrayTri[i].p1 == point1) &&
				(m->arrayTri[i].p2 == point2) && (m->arrayTri[i].p3 = point3)){
...
void vertice_tri (Mesh m, int v){
	...
	printf("El vértice #%d con coordenadas x = %.4f y = %.4f z = %.4f\n", v, m->arrayCord[v].x,
			m->arrayCord[v].y, m->arrayCord[v].z);
...
void borrar_puntos (Mesh m, int p){
	...
	if (m->nVert == 0){
		...
		printf("El índice es más grande que la cantidad de puntos\n");
		return;
	}
	m->nVert--;
	return;
...
void borrar_triangulos (Mesh m, int t){
	int i;
	if (m->nFaces == 0){
		printf("No hay triángulos que borrar\n");
		return;
	} else if (t < m->nFaces){
		...
		printf("El índice es más grande que la cantidad de triángulos\n");
		return;
	}
	m->nFaces--;
	return;
